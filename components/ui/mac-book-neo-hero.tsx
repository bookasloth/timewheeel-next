...
export function FrameSequenceHero({
  ...
  const onScroll = () => {
    // Progress is measured relative to the component's own root, not the window,
    // so it works whether this is a full-page hero or a section mid-page.
    ...
    loop();
    setProgress(p);
    setNavScrolled(p > 0.002);
    setSubHidden(p > 0.004);
    let idx = -1;
    let local = 0;
  ...
  return (
    <div ref={rootRef} className={cx("fsh-root", className)}>
